import "server-only";
import { env } from "./env";
import type { AnalysisResult, JobMatchResult } from "@/types";
...
export async function analyzeResume(
  ...
  const systemPrompt =
    "You are an expert ATS resume analyzer and career coach. Always return valid JSON. Never use markdown or code blocks in your response.";
...
export async function rewriteResumeSection(
  ...
  const systemPrompt =
    "You are an expert resume writer and career coach. Rewrite resume sections professionally. Return ONLY the rewritten content without explanations or markdown.";
...
export async function generateCoverLetter(
  ...
  const systemPrompt =
    "You are an expert cover letter writer. Write professional, tailored cover letters. Return ONLY the cover letter text without explanations.";
